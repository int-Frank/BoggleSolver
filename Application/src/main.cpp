// Minimal SDL3 + Vulkan + Dear ImGui application.
...
int main(int, char**)
{
    ...
    SDL_Window* window = SDL_CreateWindow("Boggle Solver", 1280, 800, windowFlags);
    if (window == nullptr)
    {
    ...
    SetupVulkanWindow(wd, surface, fbWidth, fbHeight);
    ...
    while (!done)
    {
        ...
        if (fbWidth > 0 && fbHeight > 0 && (g_SwapChainRebuild || g_MainWindowData.Width != fbWidth || g_MainWindowData.Height != fbHeight))
        {
            ...
            ImGui_ImplVulkanH_CreateOrResizeWindow(g_Instance, g_PhysicalDevice, g_Device, wd, g_QueueFamily, g_Allocator, fbWidth, fbHeight, g_MinImageCount, 0);
